<%-- 
    ...
    Created on : Feb 10, 2023, 11:30:30 AM
    Author     : ACER
--%>
...
<html>
    ...
    <body style="background: beige">
        ...
        <div class="container">
            <h2 style="font-weight: bold; font-size: 45px; color: #4EB748; margin-top: 2%; margin-bottom: 2%">
                Booking Motel Room
            </h2>
            <div class="row">
                <div class="col-8">
                    ...
                    <form id="form" action="createbooking" method="post"> 
                        ...
                        <div class="row">
                            ...
                            <div  class="col">
                                <select onchange="handleSelect()" id="motelId"  class="form-select form-select-lg mb-3" aria-label=".form-select-lg example">
                                    <c:if test="${lsMotel.size()>0}">
                                        <c:forEach var="d" items="${lsMotel}" >
                                            <option value="${d.motelId}" >${d.motelName}</option>
                                        </c:forEach>
                                    </c:if>
                                </select>
                            </div>
                        ...
                        <button onclick="handleSubmit()" class="btn btn-primary" style="background: #4EB748; border: none" ><b>Submit</b></button>
                    </form>
                </div>
            </div>
        </div>
        <script>
            const handleSelect = async function () {
                const motelId = document.getElementById("motelId").value;
                const floor = document.getElementById("floor").value;
                const res = await fetch('room?did=' + motelId + '&fid=' + floor).then(res => res.json());
